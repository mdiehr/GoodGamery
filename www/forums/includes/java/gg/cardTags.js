...
function getImageSource(cardname, set) {
    ...
    if (set == "CON" || set == "ME3" || set == "GPT" || set == "DIS" || set == "MM" || set == "NE" || set == "PR") {
        set = "general";
    }

    var dual_kludge = false;

    if (set == "DGM") {
        if (cardname == "steam_vents" || cardname == "overgrown_tomb" || cardname == "hallowed_fountain" || cardname == "blood_crypt" || cardname == "temple_garden") {
            set = "RTR";
            dual_kludge = true;
        } else if (cardname == "sacred_foundry" || cardname == "watery_grave" || cardname == "stomping_ground" || cardname == "godless_shrine" || cardname == "breeding_pool") {
            set = "GTC"
            dual_kludge = true;
        }
    }

    var url_value = "http://www.wizards.com/global/images/magic/" + set + "/" + cardname + ".jpg";
...
jQuery(document).ready(installCardTags);
...
function visualDecklist() {
    OpenWindow = window.open("", "newwin", "toolbar=no,menubar=no,scrollbars=yes");
    OpenWindow.document.write("<html><title>Visual Decklist</title><body>");

    var text = "";
    for (var i = 0; i < arguments.length; i++)  {
        text += "<img src=" + arguments[i] + " />";
    }

    OpenWindow.document.write(text);
    OpenWindow.document.write("</body></title>");
...
function fixPost(post) {
    ...
    innerHTML = replaceTags(innerHTML,
        ...
        for (var bnum = 0; bnum < blocks.length; bnum++) {
            ...
            for (var ix = 0; ix < lines.length; ix++) {
                ...
                if (parsed == undefined) {
                    ...
                    out_text += parsed.count + ' <a href="https://mtgcardfinder.herokuapp.com/html?card=' + cardNameSafe + '" class="jTip" name=""  onclick=\'window.open("http://scryfall.com/search?q=!%22' + cardNameSafe + '%22")\' >' + parsed.name + '</a><br/>';
        ...
        var anchor = "";
        anchor += "<a onclick=\"visualDecklist('";
        anchor += cardList.join("','");
        anchor += "')\">Visual decklist</a>";
        text += "<br>" + anchor + "<br>";
    ...
    innerHTML = replaceTags(innerHTML,
        ...
        while (input_array.length > 0) {
            ...
            while (pack_array.length > 0) {
                ...
                var card_title = escape(card).replace(/%20/g, " ");
                if (card == "") {
                    continue;
                }
                ...
                output += "<img alt='" + card_title + "' title='" + card_title + "' src='" + getImageSource(card, set) + "' style='height:143px; width:100px; margin:5px; border:4px solid " + color + ";'>";
            }
            ...
            var pick_title = escape(pick).replace(/%20/g, " ");
            var set = input_array.shift();
            output += "<img alt='" + pick_title + "' title='" + pick_title + "' src='" + getImageSource(pick, set) + "' style='height:143px; width:100px; margin:5px; border:4px solid gold;'>";
            output += "<br/><br/>";
        }
    ...
    innerHTML = replaceTags(innerHTML,
        "math", function (inner) {
        var encoded = encodeURIComponent(inner);
        url = "http://chart.apis.google.com/chart?cht=tx&chf=a,s,000000FF&chl=" + encoded;
        return "<img src='" + url + "' alt='" + inner + "'>";
    });
